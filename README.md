...
Publishing is automated: a GitHub Release (or a manual **Actions → Publish → Run workflow**) builds the sdist/wheel and uploads to PyPI from the `pypi` environment using the `PYPI_API_TOKEN` secret.
