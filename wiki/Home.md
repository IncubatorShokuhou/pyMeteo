...
Defaults: temperature `C`, pressure `hPa`, wind `m/s`, relative humidity `%`, mixing ratio `kg/kg`, distance `km` (thickness defaults to `m`), angles in degrees. Unknown strings raise `pymeteo.UnitError`. Details: [Units](Units).

```bash
pip install pymeteo-kit
```

`import pymeteo`. NCL builtin names (fixed NCL units) are in [`pymeteo.ncl`](NCL), not at the package root.

中文：[首页](Home_zh).

| Page | Contents |
|------|----------|
| [Thermo](Thermo) | Vapour, θ, θe, LCL, wet-bulb, visibility |
| [Indices](Indices) | Showalter, K, A, TT, SWEAT, LI |
| [Wind](Wind) | Speed, direction, components, bulk shear |
| [Geo](Geo) | Distance, gravity, SLP, thickness |
| [Comfort](Comfort) | NWS heat index, wind chill |
| [Dynamics](Dynamics) | Coriolis, ω ↔ w |
| [Units](Units) | Aliases and `UnitError` |
| [NCL](NCL) | NCL name shims |
| [Install](Install) | `pip install pymeteo-kit` |
