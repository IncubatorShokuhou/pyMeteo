...
A GitHub Release, or **Actions → Publish → Run workflow**, builds the sdist/wheel and uploads to PyPI from the `pypi` environment using the `PYPI_API_TOKEN` secret. Workflow: `.github/workflows/publish.yml`. See [Publishing](Publishing.md).
