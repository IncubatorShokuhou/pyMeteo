...
GitHub Release，或手动 **Actions → Publish → Run workflow**，会构建 sdist/wheel 并用 `pypi` 环境里的 `PYPI_API_TOKEN` 上传到 PyPI。工作流文件：`.github/workflows/publish.yml`。详见 [发布与贡献](Publishing_zh.md)。
